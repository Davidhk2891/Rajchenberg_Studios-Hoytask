...
class CreateTaskSetDialogViewModel @Inject constructor(
    ...
    private fun createTaskSet(title: String) = viewModelScope.launch {
        ...
        createTaskSetEventChannel.send(
            CreateTaskSetEvent.GoToSetAndShowSetCreatedConfirmationMessage(
                title,
                "Set created"
            )
        )
    ...
    private fun createTaskSetAddTaskNoNavigation(title: String) = viewModelScope.launch {
        ...
        createTaskSetEventChannel.send(
            CreateTaskSetEvent.DoNotGoToSetAndShowSetCreatedConfirmationMessage(
                "Set created with this task",
                CREATE_SET_RESULT_OK
            )
        )
    }
